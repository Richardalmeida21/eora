{% set video_link = settings.banner_video_horizontal_embed %}
{% set video_link_mobile = settings.banner_video_horizontal_embed_mobile %}

{% if (video_link or video_link_mobile) and settings.banner_video_horizontal %}
    {% set has_video_text = settings.banner_video_horizontal_button_text_title or settings.banner_video_horizontal_button_text_description %}
    
    <section class="js-section-banner-video-horizontal section-banner-video-horizontal position-relative section-home" data-store="home-banner-video-horizontal" data-transition="fade-in-up">
        <div class="banner-video-horizontal container-fluid my-5">
            {% if settings.banner_video_horizontal_url != '' %}
                <a href="{{ settings.banner_video_horizontal_url }}" class="overlay-link">
            {% endif %}
            ...
            <figure class="image -custom d-none d-md-block">
                {% if video_link %}
                    <div class="js-video home-video embed-responsive embed-responsive-21by9 position-relative" data-video-type="desktop">
                        {% if settings.banner_video_horizontal_type == 'sound' %}
                        ...
                        {# Termo "Video de" traduzido não é suficiente para acessibilidade - usar título do vídeo se possível ou fallback #}
                        {% set video_accessibility_title = settings.banner_video_horizontal_button_text_title | default(store.name) %}
                            
                            {# Otimização CLS: Adicionado width e height e style para aspect-ratio #}
                            <img src="{{ 'images/empty-placeholder.png' | static_url }}" data-src="{{ video_image_src }}" class="lazyload video-image fade-in" alt="{{ 'Video promocional de' | translate }} {{ video_accessibility_title }}" width="1200" height="675" style="width:100%; height:auto; aspect-ratio:16/9;" />
                            <div class="placeholder-fade"></div>
                        </div>

                        {# Empty iframe component: will be filled with JS on play button click #}
                        {# Adicionado title ao iframe vazio para leitores de tela quando preenchido dinamicamente #}
                        <div class="js-video-iframe embed-responsive embed-responsive-21by9" style="display: none;" data-video-color="{{ settings.accent_color | trim('#') }}" data-video-url="{{ video_link }}" title="{{ 'Player de vídeo' | translate }}">
                        </div>
                        ...
                        {% if has_video_text %}
                            <div class="banner-floating-button">
                                <div class="banner-floating-button-content">
                                    <div class="banner-floating-title">{{ settings.banner_video_horizontal_button_text_title }}</div>
                                    <svg class="banner-floating-icon" viewbox="0 0 10 10" fill="none" style="transform: scaleX(-1)">
                                        <use xlink:href="#chevron-diagonal"></use>
                                    </svg>
                                </div>
                                {% if settings.banner_video_horizontal_button_text_description %}
                                    <div class="banner-floating-description">{{ settings.banner_video_horizontal_button_text_description }}</div>
                                {% endif %}
                            </div>
                        {% endif %}
                    </div>
                    ...
                    <div class="js-video home-video embed-responsive embed-responsive-21by9 position-relative" data-video-type="mobile">
                        ...
                        {% if has_video_text %}
                            {% include 'snipplets/banner-floating-button.tpl' with {
                                'title': settings.banner_video_horizontal_button_text_title,
                                'description': settings.banner_video_horizontal_button_text_description,
                                'url': null
                            } %}
                        {% endif %}
                    </div>
