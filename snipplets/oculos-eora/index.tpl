{% set oculos_eora_category_url = settings.oculos_eora_category_url | default('') | trim %}
<link rel="stylesheet" href="{{ 'css/bolsas-eora.css' | static_url }}?v=20260922-1">
<main class="be-page" data-be-page data-search-url="{{ store.search_url | escape }}" data-category-url="{{ oculos_eora_category_url | escape }}" data-be-filter-global="EoraEyewearFilters" data-be-filter-prefix="oe" data-be-category-feed-tag="__oculos_eora_category__" data-be-all-title="Todos os óculos">
    {% if settings.oculos_eora_models %}
        <nav class="be-models" data-be-carousel aria-label="Modelos de óculos">
    ...
    <div class="be-toolbar" data-be-toolbar>
        <button class="be-button be-reset" type="button" data-be-reset aria-pressed="true">Ver todos os óculos</button>
    ...
    <section class="be-results" data-be-results hidden aria-label="Produtos de óculos" aria-busy="false">
    ...
    {% include 'snipplets/oculos-eora/banners.tpl' %}

    {% if settings.oculos_eora_best_enabled and sections.oculos_eora_best.products %}
        <section class="be-best" data-be-carousel aria-label="{{ settings.oculos_eora_best_title | default('Best sellers') | escape }}">
            <header class="be-section-heading"><h2>{{ settings.oculos_eora_best_title | default('Best sellers') | escape }}</h2></header>
            <div class="be-track" data-be-track tabindex="0" aria-label="Percorrer produtos">
                {% for product in sections.oculos_eora_best.products %}
        ...
        <dialog class="be-filter-dialog" id="be-filter-dialog" aria-labelledby="be-filter-title">
            ...
            <form data-be-filter-form>
                <div class="be-filter-dialog__body">
                    <label class="be-model-select">Modelo<select name="oe_model"><option value="">Todos os modelos</option>
                        {% for model in settings.oculos_eora_models %}
                            {% set model_tag = model.link | trim %}
                            {% set model_title = model.title | default(model_tag) | trim %}
...
<script src="{{ 'js/bolsas-eora.js' | static_url }}?v=20260923-1" defer></script>
