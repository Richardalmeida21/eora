...
{# {% set has_nav_inline_md =  settings.logo_position_desktop == 'center' and not settings.search_big_desktop  and not settings.hamburger_desktop %} #}
{% set has_nav_inline_md = not settings.search_big_desktop  and not settings.hamburger_desktop %}
...
{% set has_logo_centered_nav_below_md = not settings.hamburger_desktop %}
...
{% set logo_md_classes = settings.logo_position_desktop == 'center' ? 'col-xl order-xl-1 text-xl-center' : 'col-xl-auto order-xl-first text-xl-left' %}
{% set logo_md_padding_class = has_languages ? ' pl-xl-5' %}
{% set logo_md_spacing_classes = has_logo_centered_nav_below_md ? 'ml-md-5' ~ logo_md_padding_class : settings.logo_position_desktop == 'left' ? 'pr-xl-5' %}

{# Header visibility classes #}

{% set show_inline_desktop_hide_mobile_class = 'd-none d-xl-inline-block' %}
{% set show_inline_mobile_hide_desktop_class = 'd-inline-block d-xl-none' %}
{% set show_block_desktop_hide_mobile_class = 'd-none d-xl-block' %}
{% set show_block_mobile_hide_desktop_class = 'd-block d-xl-none' %}
...
{% set search_col_md_classes = has_nav_inline_md ? 'col-xl-auto' : has_hamburger_md_logo_centered ? 'col-xl-3 mr-xl-3' : has_logo_centered_nav_below_md ? 'col-xl-3 mr-xl-5' : 'col-xl' %}
...
{% set menu_col_md_classes = settings.logo_position_desktop == 'center' ? 'order-xl-first' : 'order-xl-last' %}
{% set menu_col_md_spacing_classes = settings.logo_position_desktop == 'center' and settings.search_big_desktop ? 'mr-md-2 pr-xl-1' %}
{% set account_col_md_classes = settings.logo_position_desktop == 'center' ? 'order-xl-1 col-3 col-xl-4' : 'col-xl-auto' %}
{% set languages_order_classes = settings.logo_position_desktop == 'center' ? 'order-first order-xl-2 mr-1 mr-xl-0' %}
...
{% set head_position = head_transparent and is_video_or_slider_above_the_fold ? 'position-fixed' : 'position-sticky' %}
{% set header_md_position = settings.head_fix_desktop ? 'position-fixed-xl' : 'position-relative-xl' %}

{% set head_transparent_background = settings.head_transparent ? 'head-transparent-only' %}

<header class="js-head-main head-main {{ header_transparent_classes }} {{ head_transparent_color_class }} {{ head_transparent_logo_class }} {{ header_colors_classes }} {{ head_position }} {{ header_md_position }} {{ header_logo_classes }} {{ header_logo_md_classes }} {{ header_logo_left_nav_below_md_classes }} {{ header_nav_inline_md_classes }} {{ header_hamburger_md_classes }} {{ header_search_big_classes }} {{ header_search_big_md_classes }} {{ head_transparent_background }} transition-soft" data-store="head">
    {# Adversiting bar #}
    {% if settings.ad_bar %}
    ...
    <div class="head-logo-row position-relative transition-soft">
        <div class="container-fluid">
            <div class="{% if not settings.head_fix_desktop %}js-nav-logo-bar{% endif %} row no-gutters align-items-center">
                ...
                <div class="{{ menu_col_classes }} {{ menu_col_md_classes }} {{ menu_col_md_spacing_classes }} col-xl-4 col-utility {% if not settings.hamburger_desktop %}d-xl-none{% endif %} d-flex">
                    {% include "snipplets/header/header-utilities.tpl" with {use_menu: true} %}

                {# Search: Icon or box #}
                    <div class="js-utility-col js-search-utility col-auto desktop-utility-col col-utility {% if settings.logo_position_mobile == 'left' %}order-1{% endif %} order-md-0">
                        ...
                        {# {% if not settings.search_big_desktop %} #}
                            <span class="{{ show_inline_desktop_hide_mobile_class }} d-flex d-md-none {% if settings.logo_position_desktop == 'left' %}float-xl-right{% endif %}">
                                {% include "snipplets/header/header-utilities.tpl" with {use_search: true} %}
                            </span>
                        {# {% endif %} #}
                        {# Mobile toggle search #}
                        {# <span class="{{ show_inline_mobile_hide_desktop_class }}">
                            {% include "snipplets/header/header-utilities.tpl" with {use_search_with_toggle: true} %}
                        </span> #}
                    </div>
                </div>
                {# Logo #}
                ...
                {% if has_nav_inline_md %}
                    ...
                    <div class="js-desktop-nav-col desktop-nav-col transition-soft col {{ show_inline_desktop_hide_mobile_class }} align-items-center pr-xl-5">
                ...
                <div class="js-utility-col col-utility desktop-utility-col text-right order-last {{ account_col_md_classes }} col-4">
    ...
    {# {% if not settings.hamburger_desktop and (settings.logo_position_desktop == 'center' or (settings.logo_position_desktop == 'left' and settings.search_big_desktop)) %} #}
        ...
        {# <div class="head-nav d-none d-xl-block">
