<div class="js-cart-item {% if item.product.is_non_shippable %}js-cart-item-non-shippable{% else %}js-cart-item-shippable{% endif %} cart-item row no-gutters {% if cart_page %}align-items-md-center{% endif %}" data-item-id="{{ item.id }}" data-store="cart-item-{{ item.product.id }}" data-component="cart.line-item">
  ...
  <div class="col-auto">
    <a href="{{ item.url }}" class="d-block cart-item-image-col {% if cart_page%}cart-item-image-col-md{% endif %} text-center">
      <figure class="image -custom">
        <img src="{{ item.featured_image | product_image_url('medium') }}" class="{% if cart_page %} cart-item-image-md{% endif %}" />
      </figure>
    </a>
  </div>
      ...
      <div class="row h-100 justify-content-between">
        ...
        <div class="col-12 {% if cart_page %}col-md-4 d-md-flex mb-md-0{% endif %} mb-1">
          ...
          <div class="row w-100 no-gutters {% if cart_page %}align-items-md-center{% endif %}">
            ...
            <div class="cart-item-name-container col-12 {% if cart_page %}col-md-8 font-md-body d-md-block mb-2{% endif %}" data-component="line-item.name">
            ...
            {% if not cart_page %}
              {# Cart item subtotal #}
              <span class="col-12 cart-item-subtotal">
                <span class="js-cart-item-subtotal font-weight-bold {% if cart_page %}d-md-flex align-self-center{% endif %}" data-line-item-id="{{ item.id }}" data-component="subtotal.value" data-component-value="{{ item.subtotal | money }}">
                  {{ item.subtotal | money }}
                </span>
              </span>
            {% endif %}

            <div class="short_variant_name col-12" data-component="name.short-variant-name">
        ...
        <div class="cart-item-quantity col-8 mt-auto{% if cart_page %} px-md-0{% endif %}" data-component="line-item.subtotal">
          {% set cart_qty_margin = '' %}
          {% if cart_page %}
        ...
        {# Cart item delete #}
        <div class="col-4 mt-auto {% if cart_page %}d-md-none{% endif %} text-center" >
          <button type="button" class="btn btn-link remove font-small pr-0 pl-2" onclick="LS.removeItem({{ item.id }}{% if not cart_page %}, true{% endif %})" data-component="line-item.remove">
            REMOVER
          </button>
        </div>
