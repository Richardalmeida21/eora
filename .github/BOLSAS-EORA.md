...
O clique em um modelo ou em Aplicar filtros rola ate os resultados, descontando a altura do cabecalho fixo e respeitando movimento reduzido. As fotos dos produtos, inclusive no hover, usam `object-fit: contain` para manter a imagem inteira. Os banners também usam `object-fit: contain` e carregam diretamente o arquivo original enviado, sem miniaturas responsivas, zoom ou recorte; quando a proporção não coincide com o espaço, o fundo do banner aparece nas sobras. Somente os banners selecionados saem dos templates inertes para a grade; as demais imagens não carregam antecipadamente. A nitidez final depende da resolução do arquivo enviado.
...
- O clique no modelo mantém o visitante na campanha e atualiza `?tag=...`. Voltar/Avançar e links compartilháveis restauram a seleção. Ver todas as bolsas/Limpar filtros restaura a uniao das tags. Ordenacao, filtros por caracteristicas e preco funcionam tanto em Todos quanto em um modelo. Quando há modelo, característica, preço ou ordenação selecionada, todas as páginas da busca são percorridas automaticamente e todos os resultados aparecem sem exigir **Mostrar mais produtos**.
- Ver todas as bolsas permanece visivel, inclusive na entrada sem filtro, com botao de 16 px e altura minima de 48 px. A rolagem conserva a barra de acoes abaixo do cabecalho fixo. Em Todos, preço, A–Z e mais novos são reorganizados sobre o conjunto completo depois da agregação; mais vendidos intercala a classificação nativa de cada modelo.
...
- O JS principal e o CSS usam `?v=20260921-16`, e o script de opções usa `?v=20260918-5`. Publicar juntos configurações, `banners.tpl`, `product-card.tpl`, CSS, scripts e `index.tpl` para atualizar também as URLs do cache.
