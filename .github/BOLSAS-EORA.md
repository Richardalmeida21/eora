...
4. Em **Banners**, logo abaixo de **Filtros por modelo**, adicione as imagens e clique no lápis. No novo campo **Filtro do banner**, informe a tag do modelo, como `maxivertice`. Para o mesmo banner também aparecer em **Todos**, informe `maxivertice | todos`. O separador é uma barra vertical e a palavra `todos` pode ser escrita em maiúsculas ou minúsculas.
5. O campo **Link ao clicar no banner** volta a receber o destino real, como `/bolsas/maxivertice/` ou uma URL completa. **Título**, **Descrição**, **Botão** e **Cor** ficam livres para o conteúdo visual do banner. Se houver Link, toda a imagem é clicável; os textos preenchidos aparecem sobre ela. A mesma imagem atende computador e celular; proporção sugerida 1000 × 1300.
6. Arraste as imagens para ordenar. Em **Todos**, aparecem todos os banners cujo novo campo termina com `| todos`. Ao selecionar um modelo, aparecem todos os banners com a tag correspondente, ignorando maiúsculas/minúsculas e espaços nas pontas. Em ambos os casos, os banners alternam entre a direita e a esquerda. Sem correspondência, ficam somente os produtos.
7. Compatibilidade de migração: enquanto um banner antigo estiver sem **Filtro do banner**, o código ainda interpreta o Link antigo como tag e `SIM` no Botão como a seleção de Todos, sem transformar a tag em link. Depois de copiar a tag para o campo novo, preencha o Link de destino e restaure o texto real do Botão.
...
11. Confira o conteúdo completo na prévia da Nuvemshop antes de publicar o tema. Em especial, confirme que o editor desta loja exibe e salva **Filtro do banner**, pois esse campo adicional não pode ser comprovado pelo ambiente Twig local.
...
- O JS principal e o CSS usam `?v=20260921-10`, e o script de opções usa `?v=20260918-5`. Publicar juntos configurações, `banners.tpl`, CSS, scripts e `index.tpl` para atualizar também as URLs do cache.
- Antes da publicação: verificar no editor que a galeria exibe e conserva o texto do novo campo **Filtro do banner**; testar o link clicável, os conteúdos opcionais, o TPL real, as imagens configuradas, mais de uma página de resultados e os filtros habilitados na loja.
